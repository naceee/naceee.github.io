...
def graph_style_and_save(ax, save_name, title=None, y_label=None, x_label=None, grid_axis="both"):
    # Title & Labels
...
def all_time_leaderboard():
    ...
    for player in PLAYERS:
        ...
        plt.plot(x, y, color=COLORS[player], label=player, linewidth=2,
                 marker=MARKERS[player], markevery=markers,
                 zorder=1/y[-1])

    graph_style_and_save(ax, "vecna_lestvica", title="Večna lestvica",
                         y_label="Skupno število točk", x_label="Odigrane runde")
...
def number_of_places():
    ...
    ax.bar(sorted_players, third, bottom=first + second, color=colors[2], label="3. mesto")
    ax.bar(sorted_players, fourth, bottom=first + second + third, color=colors[3], label="4. mesto")

    # Add Data Labels Inside Bars
    for i, (p1, p2, p3, p4) in enumerate(zip(first, second, third, fourth)):
        plt.text(i, p1 / 2, f"{round(p1, 1)}%", ha="center", va="center", fontsize=12, color="black")
        plt.text(i, p1 + p2 / 2, f"{round(p2, 1)}%", ha="center", va="center", fontsize=12, color="black")
        plt.text(i, p1 + p2 + p3 / 2, f"{round(p3, 1)}%", ha="center", va="center", fontsize=12, color="black")
        plt.text(i, p1 + p2 + p3 + p4 / 2, f"{round(p4, 1)}%", ha="center", va="center", fontsize=12, color="black")

    graph_style_and_save(ax, "delez_uvrstitev", title="Delež uvrstitev",
                         y_label="Delež iger", grid_axis="y")
...
def wins_over_time():
    ...
    for player in PLAYERS:
        y = data[f"{player}_y"]
        x = list(range(len(y)))
        plt.plot(x, y, color=COLORS[player], label=player, linewidth=2, zorder=1/y[-1])

    graph_style_and_save(ax, "zmage_skozi_cas", title="Večna lestvica zmag",
                         y_label="Število zmag", x_label="Število iger")
